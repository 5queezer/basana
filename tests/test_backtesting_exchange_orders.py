...
from .backtesting_exchange_orders_test_data import test_order_requests_data, test_invalid_requests_data, orcl_p, aapl_p
...
@pytest.mark.parametrize(
    ...
    [
        ...
        requests.StopLimitOrder(OrderOperation.BUY, orcl_p, Decimal(1000), Decimal("1"), Decimal("1")),
        requests.StopLimitOrder(OrderOperation.SELL, orcl_p, Decimal(1), Decimal("1"), Decimal("1")),
    ],
)
...
def test_small_fill_is_ignored_after_rounding(backtesting_dispatcher):
    ...
    bs = event.FifoQueueEventSource(
        events=[
            ...
            bar.BarEvent(
                ...
                bar.Bar(dt.local_datetime(2000, 1, 4), p, Decimal(2), Decimal(2), Decimal(2), Decimal(2), Decimal(10)),
            ),
        ]
    )
    e.add_bar_source(bs)
    ...
    async def impl():
        created_order = await e.create_order(requests.LimitOrder(OrderOperation.BUY, p, Decimal("0.1"), Decimal("2")))
        await backtesting_dispatcher.run()
...
def test_liquidity_is_exhausted_and_order_is_canceled(backtesting_dispatcher):
    ...
    async def impl():
        ...
        created_order_1 = await e.create_order(requests.MarketOrder(OrderOperation.BUY, orcl_p, amount_1))
        # Should get canceled because all liquidity was consumed by the previous order.
        amount_2 = Decimal(1)
        created_order_2 = await e.create_order(requests.MarketOrder(OrderOperation.BUY, orcl_p, amount_2))
        await backtesting_dispatcher.run()
...
def test_balance_is_on_hold_while_order_is_open(backtesting_dispatcher):
    async def impl():
        ...
        created_order_2 = await e.create_order(
            requests.LimitOrder(OrderOperation.BUY, orcl_p, Decimal(1), Decimal(200))
        )
        assert (await e.get_balance("ORCL")).available == Decimal(0)
        ...
        with pytest.raises(exchange.Error):
            await e.create_order(requests.LimitOrder(OrderOperation.BUY, orcl_p, Decimal("1"), Decimal(760)))
